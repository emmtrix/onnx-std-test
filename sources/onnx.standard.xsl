...
<!--
  ONNX house style for PDF output (XSL-FO), used by the Metanorma `generic`
  flavour via `pdf_stylesheet` in sources/onnx.yml.

  PROVENANCE
  ==========
  Derived from cc.standard.xsl of metanorma-cc 2.9.0 (CalConnect flavour),
  which is published under the same terms as Metanorma itself. The `generic`
  flavour ships no XSL-FO stylesheet of its own, so a working one was taken as
  the starting point for the ONNX house style.

  CHANGES FROM UPSTREAM
  =====================
  All four publisher-specific places were made document-driven, so that the
  publisher identity continues to live in sources/onnx.yml alone and this file
  needs no edit when it changes:

    1. Cover copyright  - owner read from bibdata/copyright/owner/organization,
                          instead of a hardcoded organization name.
    2. Running header   - docidentifier[@primary='true'] instead of
                          docidentifier[@type='CalConnect'].
    3. Draft warning    - reworded to be publisher-neutral. The upstream text
                          repeated its final sentence verbatim in the block
                          that follows; that duplication was dropped.
		...
		<xsl:for-each select="//mn:metanorma">
			...
			<xsl:for-each select="xalan:nodeset($current_document)">
				<mnx:doc num="{$num}">
					...
					<xsl:variable name="header" select="/mn:metanorma/mn:bibdata/mn:docidentifier[@primary = 'true']"/>
						...
						<fo:block-container font-size="10pt" border="0.5pt solid black" margin-bottom="12pt" margin-left="-1mm" margin-right="-1mm">
							<fo:block-container xsl:use-attribute-sets="reset-margins-style">
								...
								<fo:block margin-left="2mm" margin-right="2mm">
									<fo:block margin-bottom="6pt">This document is a draft and is not a standard. It is distributed for review and comment, is subject to change without notice, and may not be referred to as a standard.</fo:block>
